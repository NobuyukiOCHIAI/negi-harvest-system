#!/usr/bin/env python3
import sys
sys.path.insert(0, "/home/love-media/www/greenfarm/forecast/api/lib")
...
import pandas as pd
import joblib
import xgboost as xgb  # XGBoost対応
...
def compute_and_update_features(cycle_id):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT c.*, b.group_type FROM cycles c
                JOIN beds b ON c.bed_id=b.id WHERE c.id=%s
            """, (cycle_id,))
            cycle = cur.fetchone()
            if not cycle:
            ...
            cur.execute("""
                ...
                FROM cycles WHERE season_flag=%s AND harvest_start IS NOT NULL
            """, (season_flag,))
            row = cur.fetchone()
            base_growth_days = row["avg_days"] if row["avg_days"] is not None else DEFAULT_BASE_GROWTH_DAYS[season_flag]

            expected_harvest = plant_date + timedelta(days=int(base_growth_days)) if plant_date and base_growth_days else None
            sales_adjust_days = (harvest_start - expected_harvest).days if harvest_start and expected_harvest else None
            ...
            cur.execute("""
                ...
                LEFT JOIN (
                    ...
                    FROM harvests GROUP BY cycle_id
            ...
            prev_start = plant_date - timedelta(days=370)
            prev_end = plant_date - timedelta(days=360)
            cur.execute("""
                SELECT AVG(h.total_yield) AS avg_yield,
                       AVG(DATEDIFF(c.harvest_start, c.plant_date)) AS avg_days
                FROM cycles c
            ...
            yield_diff_prev = similar_yield - prev_yield if similar_yield is not None and prev_yield is not None else None
            days_diff_prev = similar_days - prev_days if similar_days is not None and prev_days is not None else None
            ...
            if plant_date and temp_end:
                cur.execute("""
                    ...
                    FROM weather_daily WHERE date BETWEEN %s AND %s
                """, (plant_date, temp_end))
                temps = cur.fetchone()
            else:
                temps = {k: None for k in ["avg_t", "max_t", "min_t", "std_t", "range_avg", "range_std"]}

            cur.execute("""
                UPDATE cycles SET
                    base_growth_days=%s, expected_harvest=%s, sales_adjust_days=%s,
                    similar_bed_avg_yield=%s, similar_bed_avg_days=%s,
                    prev_year_yield=%s, prev_year_days=%s,
                    yield_diff_prev=%s, days_diff_prev=%s,
                    temp_avg=%s, temp_max=%s, temp_min=%s, temp_std=%s,
                    temp_range_avg=%s, temp_range_std=%s,
                    season_flag=%s WHERE id=%s
            """, (
                base_growth_days, expected_harvest, sales_adjust_days,
                similar_yield, similar_days, prev_yield, prev_days,
                yield_diff_prev, days_diff_prev,
                temps["avg_t"], temps["max_t"], temps["min_t"], temps["std_t"],
                temps["range_avg"], temps["range_std"],
                season_flag, cycle_id
            ))
            conn.commit()
...
def predict(features, apply_partial=False, partial_yield=None, partial_ratio=None):
    model_days = joblib.load("model_days_integrated.pkl")
    model_yield = joblib.load("model_yield_integrated.pkl")

    cols = [
        "base_growth_days", "similar_bed_avg_yield", "similar_bed_avg_days",
        "prev_year_yield", "prev_year_days", "yield_diff_prev", "days_diff_prev",
        "temp_avg", "temp_max", "temp_min", "temp_std",
        "temp_range_avg", "temp_range_std", "season_flag"
    ]
    ...
    if apply_partial and partial_yield is not None and partial_ratio not in (None, 0):
        alpha = 0.5
...
def main():
    ...
    result = {
        ...
        "expected_harvest_date": features["expected_harvest"].strftime("%Y-%m-%d") if features["expected_harvest"] else None,
