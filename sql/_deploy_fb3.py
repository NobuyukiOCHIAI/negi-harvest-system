# -*- coding: utf-8 -*-
"""Upload FB3-related forecast files to Sakura."""
...
def main() -> int:
    ...
    sftp = ssh.open_sftp()
    for rel in FILES:
        lp = LOCAL / rel.replace("/", "\\")
        rp = f"{REMOTE}/{rel}"
        sftp.put(str(lp), rp)
        print("PUT", rel, lp.stat().st_size)
    sftp.close()
    _i, o, e = ssh.exec_command(
        f"php -l {REMOTE}/capacity.php; php -l {REMOTE}/inventory.php; "
        f"php -l {REMOTE}/plan.php; php -l {REMOTE}/today.php; "
        f"grep -c '週次シグナル' {REMOTE}/capacity.php || true; "
        f"grep -c '累計余剰' {REMOTE}/capacity.php || true"
    )
